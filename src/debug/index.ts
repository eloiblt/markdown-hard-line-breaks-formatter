...
async function test() {
  try {
    ...
    const markdown = await fs.readFile(path.join(__dirname, '../../../src/debug/test.md'), "utf8");

    let res = await formatWithRemark(markdown);
    res = replaceSoftBreaks(res);

    console.log(res);
    await fs.writeFile(path.join(__dirname, '../../../src/debug/test-formatted.md'), res, "utf8");
  } catch (error) {
