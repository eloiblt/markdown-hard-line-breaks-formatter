    ...
    .use(remarkStringify, {
      ...
      handlers: {
        ...
        link: (node, parent, context, safeOptions) => {
          ...
          const linkText = node.children.map(child => child.value || '').join('');
...
function replaceSoftBreaks(text) {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (currentLine.match(/^\s*-{3,}\s*$/) || nextLine.match(/^\s*-{3,}\s*$/)) {
        result.push(currentLine);
        continue;
      }

      // Si la ligne actuelle se termine par un caractère non-espace
...
async function test() {
  try {
    ...
    let res = await formatWithRemark(markdown);
    res = replaceSoftBreaks(res);

    console.log(res);
    await fs.writeFile("./src/test-formatted.md", res, "utf8");
  } catch (error) {
